...
trait Enum { //DIY enum type
  ...
  def values: Vector[EnumVal] =
    _values.get //Here you can get all the enums that exist for this type
  ...
  protected trait Value extends Ordered[Value] { self: EnumVal ⇒ //Enforce that no one mixes in Value in a non-EnumVal type
    ...
    override def toString = value //And that name is used for the toString operation
    override def equals(other: Any) = this eq other.asInstanceOf[AnyRef]
    override def hashCode = 31 * (this.getClass.## + value.## + ordinal)
  }
}
class ParseException(message: String) extends EnumValueParseException(message)
class EnumValueParseException(message: String) extends Exception(message)
